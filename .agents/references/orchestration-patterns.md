...
Plugin subagents go in `.agents/agents/` at the plugin root. This repo is a plugin (`.claude-plugin/plugin.json`), so `.agents/agents/code-reviewer.md`, `.agents/agents/security-auditor.md`, and `.agents/agents/test-engineer.md` are auto-discovered when the plugin is enabled. No path configuration needed.
