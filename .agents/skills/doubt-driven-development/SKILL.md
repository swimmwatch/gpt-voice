...
description: Use only when the user explicitly requests adversarial review or a named high-risk decision needs an independent challenge; never for every unfamiliar task.
---
...
In Claude Code, the role-based reviewers in `.agents/agents/` start with isolated context by design and are usable here — see `.agents/agents/` for the roster and per-domain match.
