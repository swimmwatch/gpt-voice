...
describe('audioEncoding', () => {
  it('encodes mono float samples as a PCM16 WAV file', () => {
    const wav = encodePcm16Wav([new Float32Array([-1, 0, 1])], TEST_SAMPLE_RATE);
    ...
    assert.equal(view.getUint32(24, true), TEST_SAMPLE_RATE);
  ...
  it('rejects invalid PCM input', () => {
    assert.throws(() => encodePcm16Wav([], TEST_SAMPLE_RATE), /at least one channel/);
    assert.throws(() => encodePcm16Wav([new Float32Array()], TEST_SAMPLE_RATE), /audio samples/);
    assert.throws(
      () => encodePcm16Wav([new Float32Array([0]), new Float32Array([0, 1])], TEST_SAMPLE_RATE),
      /channel lengths/,
    );
  ...
  it('falls back to the original blob when Web Audio is unavailable', async () => {
    ...
    const payload = await prepareTranscriptionAudio(new Blob([source], { type: WEBM_OPUS_TRANSCRIPTION_MIME_TYPE }));

    assert.equal(payload.mimeType, WEBM_OPUS_TRANSCRIPTION_MIME_TYPE);
