...
function readFloatingListStyles(): string {
  return readRendererSource('floating-list-styles.ts');
}
...
describe('interactive cursor contracts', () => {
  it('uses pointer cursors for reusable click controls', () => {
    ...
    const floatingListStyles = readFloatingListStyles();
    ...
    assert.match(dropdownMenu, /DropdownMenuItem[\s\S]*?FLOATING_LIST_ITEM_CLASS/u);
    assert.match(select, /SelectTrigger[\s\S]*?cursor-pointer/u);
    assert.match(select, /SelectItem[\s\S]*?FLOATING_LIST_ITEM_CLASS/u);
    assert.match(floatingListStyles, /FLOATING_LIST_ITEM_CLASS[\s\S]*?cursor-pointer/u);
  ...
  it('keeps disabled shared controls visibly non-actionable', () => {
    ...
    assert.match(dropdownMenu, /DropdownMenuItem[\s\S]*?FLOATING_LIST_ITEM_CLASS/u);
    assert.match(select, /SelectItem[\s\S]*?FLOATING_LIST_ITEM_CLASS/u);
    assert.match(floatingListStyles, /data-\[disabled\]:cursor-not-allowed/u);
