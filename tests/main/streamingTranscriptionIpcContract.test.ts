...
describe('trusted streaming transcription IPC integration', () => {
  ...
  it('cancels before browser teardown, provider mutation, and application quit', () => {
    ...
    const shutdownStart = browser.indexOf('export async function shutdownBackgroundBrowser');
    ...
    const switchStart = browser.indexOf('export async function switchProvider');
    const switchShutdown = browser.indexOf('await shutdownBackgroundBrowser();', switchStart);
