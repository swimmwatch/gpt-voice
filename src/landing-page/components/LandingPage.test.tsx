...
import { englishContent, getLocaleDefinition, localeRegistry } from '@landing/content';
import { LandingPage } from '@landing/components/LandingPage';

function renderEnglishLanding(): string {
  return renderToStaticMarkup(<LandingPage content={englishContent} locale={getLocaleDefinition('en')} />);
}

describe('LandingPage', () => {
  it('renders the complete English information architecture as semantic HTML', () => {
    const markup = renderEnglishLanding();
    ...
    expect(markup).toContain('class="landing-loader"');
    expect(markup).toContain('data-landing-loader="true"');
    ...
    expect(markup).toContain('aria-label="Website language: English"');
    expect(markup).toMatch(/<footer[\s\S]*?<a href="\/gpt-voice\/docs\/">Documentation<\/a>[\s\S]*?<\/footer>/);
    expect(markup).toContain('class="site-footer-logo"');
    expect(markup).toContain('src="/gpt-voice/generated/icons/gpt-voice.png"');
    expect(markup).toContain('aria-hidden="true" class="site-footer-signal"');
    expect(markup).toContain('data-footer-waveform="recording"');
    expect(markup.match(/data-footer-waveform-bar="true"/g)).toHaveLength(61);
    expect(markup).toContain('class="locale-no-js"');
    for (const locale of localeRegistry) {
      expect(markup).toContain(`href="${locale.route}"`);
  ...
  it('renders the native English demo video without autoplay', () => {
    const markup = renderEnglishLanding();
    ...
    expect(markup).not.toContain('data-demo-transcript');
    expect(markup).not.toContain('data-demo-visual-note');
  });
  ...
  it('renders the SVG-reference provider signal map with branded cards and branched routes', () => {
    ...
    expect(markup.match(/data-provider-route-input="true"/g)).toHaveLength(1);
    expect(markup).not.toContain('data-provider-route-arrow');
    ...
    expect(markup.match(/data-provider-route-target=/g)).toHaveLength(4);
    expect(markup.match(/data-provider-route-path=/g)).toHaveLength(3);
    expect(markup.match(/class="provider-route-pulse"/g)).toHaveLength(6);
