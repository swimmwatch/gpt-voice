...
export interface NativeRuntimeLogStreamDecoderDependencies {
  /** Binds accepted stderr records to the exact private launch instance when one is known. */
...
export class NativeRuntimeLogStreamDecoder {
  ...
  private consumeLine(rawLine: Buffer): void {
    ...
    if (
      !record ||
      (this.dependencies.expectedProcessInstanceId &&
        record.processInstanceId !== this.dependencies.expectedProcessInstanceId)
    ) {
...
export class NativeRuntimeLogRelay {
  ...
  public attach(forwarder: NativeRuntimeLogForwarder): void {
    ...
    for (const record of this.pending) forwarder.forward(record);
