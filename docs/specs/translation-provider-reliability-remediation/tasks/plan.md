...
| [06 Qualify supported packaged platforms](06_qualify_supported_packaged_platforms.md)                             | Representative Linux x64 and Windows x64 packages complete sanitized timeout, suspend, provider-success, tray-activity, Google overwrite/reuse, and before/after latency evidence; all gaps remain explicit | 09           | `OUT-001`–`OUT-004`; `TIME-004`–`TIME-005`; `PERF-001`, `PERF-004`–`PERF-005`, `PERF-008`; `LIFE-003`–`LIFE-009`; `SEC-001`, `SEC-006`–`SEC-010`; `COMP-001`–`COMP-002`, `COMP-006`; `UX-001`; `ACC-013`–`ACC-016`, `ACC-021`–`ACC-024`                                                                                                                                  |

## Sequencing

```text
01 -> 02 -> 03 -> 04 -> 05 -> 07 -> 08 -> 09 -> 06
```
...
Coverage audit: all 105 active requirement and acceptance IDs have at least one
