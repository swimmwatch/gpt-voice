...
| [11 Shared Translation browser context](11_shared_translation_browser_context.md)                                 | One main-process Translation context/page coordinator serializes all providers, replaces the page and resets session state only on provider-ID change, and safely quarantines uncertain cleanup                                       | 10           | `ARCH-011`; `CONC-010`; `LIFE-010`; `FAIL-012`; `SEC-011`; `ACC-026`                                                                                                                                                                                                                                                                                                     |
| [06 Qualify Linux packaged platform](06_qualify_supported_packaged_platforms.md)                                  | A representative Linux x64 package completes sanitized timeout, suspend, provider-success, tray-activity, Google overwrite/reuse, shared-context switching, and before/after latency evidence; Windows manual qualification is deferred | 11           | `OUT-001`–`OUT-004`; `TIME-004`–`TIME-005`; `PERF-001`, `PERF-004`–`PERF-005`, `PERF-008`; `LIFE-003`–`LIFE-010`; `SEC-001`, `SEC-006`–`SEC-011`; `COMP-001`–`COMP-002`, `COMP-006`; `UX-001`; `ACC-013`–`ACC-016`, `ACC-021`–`ACC-026`                                                                                                                                  |

## Sequencing

```text
01 -> 02 -> 03 -> 04 -> 05 -> 07 -> 08 -> 09 -> 10 -> 11 -> 06
```
...
- Task 11 converts provider-owned browser resources into one shared Translation
  context/page coordinator. It must complete before supported-platform qualification
  can observe provider-switch session reset and global resource serialization.
- Task 06 contains the approved Linux external, packaged, suspend/resume, and
  live-provider `MANUAL GATE`. Windows packaged qualification is deferred and must
  not be represented as completed or verified by this packet.

Coverage audit: all 115 active requirement and acceptance IDs have at least one
