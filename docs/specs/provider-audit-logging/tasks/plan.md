...
| [07 Diagnostic capture storage](07_build_diagnostic_capture_storage.md)                         | Versioned redaction and strict diagnostic tables share `gpt-voice.sqlite3` with bounded retention and deletion primitives                                   | 01                 | `ARCH-004`; `DATA-003`; `DATA-007`–`DATA-008`; `OPS-004`; `SEC-005`; `SEC-007`; `FAIL-002`–`FAIL-003`; storage portion of `COMP-003`                                                                                                           |
...
- Provider and service modules own semantic phase/cause mappings but cannot
  ...
  output to the sink.
- The shared app-database coordinator owns migration, connection, permissions,
  pruning, snapshot, purge, and shutdown ordering. Diagnostic queries never
  join or delete transcription history.
