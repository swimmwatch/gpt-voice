# 12 Create Diagnostics Analysis Skill
...
- Packet 10 is complete and the archive schema-v1 manifest, fixed members,
...
- Detect duplicate `(operationId, sequence)` events and duplicate action IDs as
  integrity contradictions, even though packet 10 normally deduplicates audit
  output.
...
- `archiver` from packet 10 is creation-only and must not validate or extract
...
- Rollback removes only the repository-local skill and its focused tests. It
  does not change exported archives, application data, logs, packet-10 runtime
...
- Packet 10 archive schema and synthetic fixture builders.
...
1. Check only packet 12 in `tasks/todo.md`.
