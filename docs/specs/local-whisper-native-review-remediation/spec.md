...
Approval: **APPROVAL-008** — the user explicitly authorized the closed Windows execution-input and installed-runtime proof remediation after approving the MSVC 19.51 Windows-evidence amendment on 2026-08-14.

Previous approvals, including **APPROVAL-007** for MSVC 19.51 executable evidence, **APPROVAL-006** for native structured logging, and **APPROVAL-005** for the consolidated two-runner and parameterized-workflow baseline, are retained as historical evidence superseded by this amendment.
...
**CMP-005:** MSVC 19.51 is the sole supported Windows C++ compiler for workstream-owned executable evidence. Windows coverage SHALL be closed through real MSVC 19.51 builds, MSVC code analysis, a dedicated MSVC ASan configuration, deterministic platform tests, CPU runtime builds, and PE inspection. The CUDA 12.8.1 and AMD Vulkan preview profiles that remain pinned to MSVC 19.39 are contract-only, pending qualification, and SHALL NOT compile, execute, package, or satisfy Windows native evidence in this workstream. They SHALL NOT be relabeled to MSVC 19.51 because CUDA 12.8 supports only the MSVC 193x Windows host-compiler family. No unsupported compiler override, `clang-cl` toolchain, runtime, package, or qualification claim is introduced.
...
**CMP-010:** Runner operating-system selection and compiler selection are independent contracts. Every lane SHALL provision and verify the pinned compiler profile required by this specification rather than accept an unverified image default. The Windows executable-evidence lane SHALL use MSVC 19.51 from Visual Studio 2026 and SHALL reject another compiler family or minor release. Before executable or package evidence, the lane SHALL derive a private candidate from the pending static CPU profile by version-checking and hashing the exact prepared compiler, linker, archiver, PE inspector, CMake, Ninja, Windows SDK library, and locked VC Runtime DLL bytes used by that execution. The network-denied build SHALL apply the Windows Firewall boundary to that exact executable set and prove denied egress with the same Node executable that owns the boundary; an unrelated contract-only probe SHALL NOT substitute. VC Runtime materialization SHALL use either the checksum-locked installer or an explicit installed-runtime proof that requires the exact registry version, every allowlisted DLL digest, and valid Microsoft Authenticode metadata. The current checksum-locked redistribution license remains mandatory in both modes. Build-tool license documents are compliance records, not executable inputs, and SHALL NOT be relabeled as build authority. An unavailable or unreadable exact input SHALL fail as missing evidence and SHALL NOT silently fall back to MSVC 19.39, another MSVC release, an ambient compiler, a version-only Ninja claim, or an unchecked system DLL. Contract-only inspection of the CUDA 12.8.1 and AMD Vulkan preview MSVC 19.39 profiles remains separately labeled and SHALL NOT satisfy executable evidence.
...
**WIN-001:** The Windows native-quality workflow SHALL perform real MSVC 19.51 builds and execute project-owned tests for:
...
Windows SHALL preserve the pinned MSVC Debug library's uniform iterator/container-debug configuration across every project-owned and GoogleTest translation unit. A different MSVC STL-hardening macro MAY be added only if supported by MSVC 19.51 and applied consistently to the complete linked test graph. The amendment SHALL NOT copy `_ITERATOR_DEBUG_LEVEL=1` or `_CONTAINER_DEBUG_LEVEL=1` into isolated targets or weaken the existing Debug level.
...
| `windows-2025` x64                        | Primary Windows runner                                      | Full ordinary MSVC 19.51 native builds/tests; MSVC analysis; dedicated MSVC ASan; CPU runtime build and PE hardening inspection; Windows C++ CodeQL; Windows package build and smoke; and the owning Windows SBOM, vulnerability, provenance, and attestation gates. CUDA 12.8.1 and AMD Vulkan MSVC 19.39 checks remain contract-only and separately labeled.              |
...
| AC-AUT-042 | Feed runner evidence wrong image metadata, an ambient compiler, executable-evidence MSVC other than 19.51, an MSVC 19.39 accelerator contract presented as executed evidence, divergent source manifests, a cancelled required platform job, a deprecated-label substitution, and a repository variable containing a latest alias. | Every case fails as missing or mismatched evidence; no fallback to an image default, contract-only accelerator profile, or latest alias occurs, and the report identifies the exact remediation boundary without claiming desktop qualification.                                                      | CMP-010, CMP-012, RUN-005–RUN-006, TST-010          |
