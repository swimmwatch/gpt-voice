# 18 Linux, Shared, And Security Remediation Gate
...
- Packets 01–17 are checked complete. Every code-bearing Packet 04–17 has successful exact-candidate CI with the required Ubuntu 24.04 and Windows Server 2025 jobs executed and no required Windows skip.
...
- Primary: OUT-001–OUT-004 and GAT-001–GAT-004 for Linux/shared and integrated automated readiness.
- Final Linux/security audit: all SCP, CMP, ARC, SEC, OPS, TST, SUP, WF, DEP, DCK, SAST, ART, VUL, ATT, REP, SRV, RUN, native-quality, and advisory requirements owned by Packets 01–17.
- Acceptance: every Linux/shared and CI-applicable portion of AC-AUT-001–AC-AUT-042; AC-MAN-001, Linux portion of AC-MAN-003, AC-MAN-005–AC-MAN-008.
...
- Final inventory of every supported-host manual Windows command, expected binary, fixture, and case reserved for Packet 19, plus the hosted-runner/container evidence needed for AC-MAN-009 comparison.
...
1. Audit the final implementation map against all 38 selected source/CI/security/runner subjects, every active requirement ID, and AC-AUT-001–AC-AUT-042 plus AC-MAN-001–AC-MAN-009. Confirm no requirement was lost during packet decomposition.
...
- Passing this packet proves Linux supported-host/manual readiness plus complete automated two-runner and security evidence. It does not satisfy Packet 19's supported-host manual Windows evidence, AC-MAN-009, or qualification.

## Expected Files Or Components

- Production/test/configuration/workflow/security files owned by Packets 01–17 only when a gate-discovered fix is required.
...
- Every applicable Linux/shared/security AC-AUT case through AC-AUT-042 passes under its intended evidence class, platform, and locked profile.
...
- The Packet 19 manual manifest is complete and no hosted Windows Server or automated result is mislabeled as supported-desktop manual evidence.
- Exclusions, privacy constraints, and no-release/no-qualification boundaries remain intact.

## Verification

Run the canonical Linux/shared/security completion commands introduced by Packets 01–17, including at minimum:
...
1. Before the candidate or any fix commit, run every applicable local check. After local/supported-host Linux verification passes, leave Packet 18 unchecked, update `handoff.md`, stage only packet-owned paths, commit conventionally, and push without force.
...
3. Every required Ubuntu 24.04 and Windows Server 2025 job must execute the complete final applicable native/C++ and security surface. The Windows native and package/security chain must conclude `success`; no required Windows skip is acceptable. This does not replace Packet 19 manual Windows cases.
...
- Keep Packet 18 unchecked until every Linux/shared/security gate passes. Record the exact requirement, acceptance ID, command, and bounded failure classification.
...
- Packet-local handoff evidence from Packets 01–17.

## Completion And Handoff

- Record all Linux/shared/security results, fixes, exact ELF/artifact/attestation identities, manual evidence, the Packet 19 manifest, candidate SHA, and successful final two-runner/security jobs in `handoff.md`.
- Check Packet 18 only after every local/manual Linux/security gate and the code-bearing exact-SHA remote gate pass with no required Windows skip.
- Set `Exact next packet: 19 — Windows Validation And Remediation Gate` and stop.
