...
- **SEC-012:** `cloakbrowser@0.5.3 -> tar@7.5.22` resolves `GHSA-r292-9mhp-454m`; no production advisory exception remains approved.
- **OPS-001:** `SECURITY.md` owns one “Known production advisory exceptions” section and explicitly records when no exception is approved.
- The production-advisory policy requires an empty npm production-audit vulnerability set and fails closed for malformed evidence, any advisory, or inconsistent `SECURITY.md` state.
