...
This is a bounded evidence index, not a process log. Before each entry, obtain
separate explicit authority and ask the user for a new finite timeout in their
language. Explain that the timeout prevents indefinite waiting and recommend the
expected duration plus a practical margin. For a process that normally takes
30 minutes, recommend approximately 40 minutes (2,400 seconds).
...
| Gate                                        | Separate authority and timeout                                                              | Required bounded evidence                                                                                                | Status  |
| ------------------------------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------- |
...
| First compatibility workflow run            | Authorize a normal safe CI run/dispatch and choose a timeout sized to its expected duration | Exact commit, run ID, all six matrix member IDs, and `Watch Process Compatibility` aggregate conclusion                  | Pending |
