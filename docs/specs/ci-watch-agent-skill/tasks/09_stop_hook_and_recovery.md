...
- IDE restart/close kills or disconnects the hook and may kill the watcher. Recovery never self-starts a new chat; explicit `resume` is required and must re-observe the exact target before reattach/repair/finalize/block. The sole `AUTH-001` alpha-release recovery may replace only a lost local CLI child after explicit resume, retain its watch/tag/source/correlation identity, and reattach the exact remote workflow before dispatch.
