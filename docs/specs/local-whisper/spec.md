...
**BASE-001** Local Whisper has an unshipped, non-production foundation: the repository contains the shared/settings/catalog contracts, managed artifact/inventory/filesystem boundaries, framed protocol, native launcher/guard, CPU/CUDA workers, process-owned production environment, runtime/device discovery, exact launch leasing, worker residency/lifecycle, safe artifact removal, and qualification schemas/tooling produced by completed packets and the in-progress Task 19 worktree. Production download/resume/retry/cancel/update is not composed, no candidate/profile/artifact set is frozen, no real production or qualification catalog is available, and no platform release-support claim has passed its full gate. Existing work remains subject to this specification and the rebuilt plan.
...
16. Release 1 qualifies exactly one model variant per family: `full` for `tiny`, `base`, `small`, and `medium`; `q5_0` for `large-v3` and `large-v3-turbo`. Other catalog variants remain `estimateOnly` until separate qualification (**MODEL-011, QUAL-003**).
17. Technical candidate qualification uses an isolated non-publishable `qualification` catalog/key purpose. Fixture, qualification, and production trust never cross; protected production signing/legal/publication evidence is reconciled only after Linux and Windows technical qualification (**PKG-011, SEC-014, OPS-003**).
18. Accuracy and direct-engine parity use one pinned public FLEURS `en_us`/`ru_ru` corpus manifest. Private user recordings are forbidden as qualification evidence (**QUAL-002, PRIV-005**).
19. Qualification evidence uses one acyclic shared-candidate/platform-branch/aggregate identity graph. Task 19 seals the shared core and Linux branch, Task 20 seals the Windows branch without mutating the core or Linux evidence, and Task 21 seals the aggregate root (**REL-001, QUAL-001, COMP-012**).
...
**CPU-001, COMP-008, AC-MAN-002** `whisperCpp` targets Production CPU support on Windows and Linux x64 only after per-OS correctness and performance qualification. This CPU contract never extends to macOS.
...
- runtime revision by `(engine, target, backend)`;
...
Full and reviewed quantized `ggml` files are distinct variants. **MODEL-011, QUAL-003** The release-1 qualified matrix is closed to `tiny/full`, `base/full`, `small/full`, `medium/full`, `large-v3/q5_0`, and `large-v3-turbo/q5_0`, all sourced from immutable commit `5359861c739e955e79d9a303bcbc70fb988958b1` of public Hugging Face repository `ggerganov/whisper.cpp`. A different upstream repository/commit/file or an additional variant requires a reviewed specification/catalog revision and its own qualification evidence.
...
**REL-001, QUAL-001** Qualification identity SHALL be one directed acyclic graph with the following strict construction order. Every digest is lowercase SHA-256 over UTF-8 canonical JSON after removing exactly that document's own digest field. Canonical JSON recursively orders object keys lexicographically, preserves only schema-defined array order, contains no insignificant whitespace or byte-order mark, and rejects unknown fields, duplicate semantic identities, non-finite numbers, and values outside the schema before hashing. Every digest-bearing array whose order has no domain meaning SHALL be sorted lexicographically by its declared stable identity before hashing.

1. The **shared candidate input** is frozen first as `candidateInputDigest`. It binds one explicitly supplied candidate SemVer and UTC freeze timestamp; exact Git source commit/tree and clean-state proof; shared engine/source/patch locks; catalog, trust-purpose, transfer-profile, redirect-policy, qualification-algorithm, and evidence-schema revisions; the exact six public model objects; FLEURS corpus/conversion/performance-fixture manifests; Task 17 fixture digest; required platform/backend matrix; and predecessor-selection rule. It SHALL NOT contain any platform package, runtime pack, direct-engine binary, platform toolchain, platform profile, measurement series, result, evidence-index, platform-graph, or aggregate digest.
2. Each platform then freezes one **platform input** as `platformInputDigest` that binds `candidateInputDigest`, exact platform/architecture, application package files and hashes, embedded qualification catalog/keyring/origin identity, runtime-pack manifests/archives/signature inputs, platform toolchain, direct-engine binaries/manifests, qualification-server identity, platform-specific notices/SBOM/provenance, and the exact predecessor package selected at the shared freeze timestamp. It SHALL NOT contain any profile, measurement, result, evidence-index, platform-graph, or aggregate digest.
3. Every immutable CPU/CUDA **qualification profile** is frozen before measurement as `profileDigest`. It binds `candidateInputDigest` and the applicable `platformInputDigest` plus the exact OS/reference-hardware/driver/runtime/backend/device-proof class, model/corpus/direct-engine identities, algorithms, tools, repetitions, units, thresholds, tolerances, and failure bounds. It SHALL NOT contain any platform-graph, measurement, result, evidence-index, or aggregate digest.
4. One **platform graph** is then frozen as `platformGraphDigest`. It binds `candidateInputDigest`, exactly one `platformInputDigest`, and the complete lexicographically sorted profile-digest set required for that platform. It SHALL NOT contain measurement-series, result, evidence-index, other-platform, or aggregate digests.
5. Platform evidence is sealed only after the platform graph: every measurement series binds `candidateInputDigest`, `platformGraphDigest`, and its applicable `profileDigest`; the platform result binds the same candidate/platform graph plus the complete sorted measurement/evidence-series digest set; and the platform evidence index binds the candidate/platform graph, platform-result digest, and every sanitized evidence entry. No earlier graph layer hashes a later evidence layer.
6. Task 21 finally freezes one **aggregate evidence root** as `aggregateEvidenceDigest`. It binds `candidateInputDigest` and, for exactly Linux x64 and Windows x64, each immutable `platformGraphDigest`, platform-result digest, and platform-evidence-index digest, plus the acceptance-ownership and qualification-to-production-delta evidence required by the aggregate gate. Platform documents SHALL NOT hash the later aggregate root.

Task 19 owns the shared candidate input and Linux branch. Task 20 SHALL consume the unchanged `candidateInputDigest` and immutable Linux branch, then add only the Windows platform input/profile/graph/result/evidence branch. Task 21 SHALL consume both branches without regenerating them. A shared-input change invalidates both platform branches and the aggregate root. A platform-input, profile, or evidence change invalidates that platform branch and the aggregate root but SHALL NOT rewrite the unchanged other-platform branch.
...
The unfrozen, uncommitted circular qualification schema-v2 shape is invalid and SHALL be replaced in place before the first freeze. The corrected v2 schema family SHALL expose distinct shared-candidate-input, platform-input, profile, platform-graph, platform-result, and platform-evidence-index document kinds with the bindings above; the measurement-series schema SHALL carry the equivalent candidate/platform/profile bindings. No v3 compatibility surface or migration is required because no circular-v2 evidence was frozen, published, or consumed. Validators SHALL reject every legacy `candidateDigest`/`profileDigests` cycle, placeholder, fixed-point attempt, missing edge, backward edge, mixed shared candidate, mixed platform branch, duplicate profile, incomplete platform matrix, or unhashed cross-document binding. This qualification-schema repair SHALL NOT change the catalog payload schema, Task 17 fixture bytes, or Task 17 fixture digest.

Qualification evidence SHALL never silently combine packages or artifacts from different shared inputs, platform branches, SemVer values, catalogs, purposes, profiles, results, or freeze records. Only a Section 12.1 authorized trust/publication-only delta may be compared later, and it still requires aggregate production-readiness validation rather than mutation of qualification evidence.
...
- `production`: only a frozen reviewed production-purpose payload/keyring/origin tuple may be packaged after protected signing, legal, hosting, and publication gates. Production packages SHALL reject fixture and qualification keys/purposes.

Key IDs, public keys, and accepted purposes SHALL be pairwise disjoint between fixture, qualification, and production. A build-mode/purpose mismatch, missing tuple, test/qualification key in production collection, production key in a qualification build, or trust-resource substitution fails before package creation or collection. The aggregate production-readiness gate SHALL prove that a production package is built from the same qualified source commit and runtime/model bytes and that any package difference is limited to declared version/trust/catalog/origin/release metadata; executable code, native packs, models, protocols, and qualification-relevant settings SHALL not change under that exception.
...
**DIST-001, DIST-002** Release-1 origins are closed as follows:

- project-built runtime packs use immutable assets under `https://github.com/swimmwatch/gpt-voice/releases/download/<exact-tag>/<exact-asset>` and a signed GitHub-release redirect policy;
...
A qualification-purpose catalog MAY replace only the runtime origin with a single-use loopback HTTPS qualification server whose generated CA/public certificate, exact origin/port, range/ETag behavior, and archive digest are pinned in the qualification record. It SHALL bind only `127.0.0.1`, require no credentials, expose only frozen candidate runtime objects, terminate with the qualification run, and is never accepted by production. This allows platform qualification to use exact runtime bytes without publishing them. After separately authorized production upload, the aggregate production-readiness gate SHALL download the identical runtime archive digest from the final GitHub Release origin and run bounded transport/install parity; it SHALL NOT reuse the loopback result as final-origin evidence.
...
- generate the qualification catalog/keyring/package from the frozen candidate record, never from a mutable working directory or moving branch. Qualification private key material remains outside repository/evidence and is destroyed or revoked after use; public key/catalog/package digests remain evidence;
- generate the production catalog/keyring/package only after both platform qualifications pass and protected production trust/legal/publication inputs are available. The aggregate production-readiness gate SHALL compare qualification and production manifests field by field and reject any undeclared executable, model, source, protocol, setting, or packaging delta.

Provenance, SBOM, notices, legal review, and signing evidence SHALL be privacy-safe and checksum-linked. A missing report, unresolved license/component, stale source URL, mutable Hugging Face revision, unsigned catalog, failed reproducibility comparison, unavailable runtime asset, or unapproved production trust input is a release blocker; it SHALL NOT be converted into synthetic `Passed` evidence. Qualification completion and production publication approval are distinct states.
...
| AC-AUTO-067 | Construct the corrected v2 shared candidate, Linux/Windows platform-input, profile, platform-graph, result, and evidence-index documents in valid order; then remove, duplicate, reorder, mix, or back-reference every digest edge, inject the legacy candidate/profile cycle and placeholders, vary SemVer/source/dirty state, and test predecessor releases before/at/after the shared freeze timestamp.                                                                                                                                                                        | Only the complete acyclic shared-core/platform-branch graph validates. Task 19 can seal Linux without Windows artifacts; Task 20 adds Windows without changing the shared input or Linux branch; Task 21 alone seals the aggregate root. Legacy circular v2 shapes, fixed-point/placeholders, unhashed bindings, mixed candidates/platforms, mutable versions, and wrong predecessors fail closed.                                                                 |
...
| AC-AUTO-071 | Compare a passing qualification candidate/package with fixture, altered-code, altered-native, altered-model, altered-setting/protocol, and production-trust/package variants; run release collection with missing and complete external gate evidence.                                                                                                                                                                                                                                                                                                                                  | Only declared SemVer/trust/catalog/origin/release-metadata deltas may be promoted; executable and qualification-relevant bytes remain identical; platform technical results stay distinct from the aggregate production/legal/publication verdict; missing external evidence remains an explicit release blocker and is never fabricated as a platform failure or pass.                                                                                  |
...
Every Production matrix cell SHALL have one checked-in, immutable profile frozen before measurements. The profile binds the Section 9.6 `candidateInputDigest` and applicable `platformInputDigest`, exact OS build/family/architecture, reference hardware, driver/runtime/ISA, engine/backend/device proof, tool versions, all six canonical model identities, corpus/direct-engine manifests, repetitions, algorithms, and pass limits. The complete profile set SHALL be sealed in `platformGraphDigest` before any measurement begins. A schema validator SHALL reject any missing algorithm, bound, unit, source identity, graph edge, or evidence digest; reviewers SHALL NOT choose or relax values after observing results.

**QUAL-002, PRIV-005** The shared accuracy corpus SHALL be derived only from public FLEURS repository `google/fleurs` commit `70bb2e84b976b7e960aa89f1c648e09c59f894dd`. Before platform execution, a checked-in manifest SHALL select at least ten `en_us` and ten `ru_ru` test clips with distinct normalized reference text and at least 120 seconds total audio per locale. It SHALL bind the upstream TSV/audio-archive URLs and hashes, selected member names and hashes, reference text, CC BY 4.0 notice, materializer source/tool version, and resulting canonical WAV hashes. A project-owned qualification-only materializer MAY use a pinned Python standard-library toolchain, but SHALL stream only exact allowlisted regular members from verified archives into a fresh temporary root, reject traversal/links/special/duplicate entries, validate mono 16-kHz signed 16-bit PCM, and emit no dependency into the shipped application. Raw corpus archives/audio remain reproducible caches, not committed platform evidence; private user audio and local source paths are forbidden.
...
**MODEL-011, QUAL-003** A Linux or Windows CPU/CUDA cell is Production-eligible only after every release-1 canonical model variant passes load, warm-up, corpus parity, resource, unload, and recovery gates on that cell. Only `base/full` carries the five-fixture RTF gate, but the other five models are not allowed to remain estimate-only under a Production cell. Missing, failed, invalid, or unpublished technical evidence keeps the cell conditional; protected production trust/legal/publication evidence is evaluated separately by the aggregate gate and never lowers a technical gate automatically.

### 19.3 Manual and hardware gates

| ID         | Environment and procedure                                                                                                                                                                                                                                                                                                                                                          | Passing condition                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-MAN-001 | Available Linux NVIDIA laptop: install the pinned `whisperCpp` CUDA pack and run all six canonical model configurations plus hybrid-device selection under the frozen Linux NVIDIA profile.                                                                                                                                                                                        | Integrity, correct device/backend, every model's load/warm-up/parity/resource/unload/recovery gates, base RTF, repeated inference, crash recovery, and cleanup all pass. Evidence applies only to Linux NVIDIA.                                                                                                                                                                                                         |
| AC-MAN-002 | Windows/Linux x64 CPU with GPU target explicitly not selected, using each OS's declared `whisperCpp` Production reference hardware/profile and all six canonical model configurations.                                                                                                                                                                                             | No GPU initializes and every model's correctness/resource/lifecycle gate plus base RTF passes before that OS CPU cell becomes Production.                                                                                                                                                                                                                                                                               |
| AC-MAN-003 | Separate representative Windows x64 NVIDIA hardware, the pinned `whisperCpp` CUDA pack, and the complete all-six-model Windows qualification profile.                                                                                                                                                                                                                              | Every Section 19.2 limit passes before the Windows NVIDIA cell is labeled Production. Linux evidence cannot substitute.                                                                                                                                                                                                                                                                                                 |
...
| AC-MAN-012 | Review runtime redistribution plus public-upstream model-download facilitation: licenses, provenance, signing, key rotation/denylist, deterministic runtime archives, pinned Node/Electron gzip implementation, SBOM/notices, FLEURS CC BY 4.0 evidence, Hugging Face/model-card terms, and production trust/publication authority.                                                | Every redistributed runtime file/codec is accounted for and legally distributable; every upstream model/corpus source and required notice is approved for the specified use; protected signing/publication evidence and checksum-linked provenance/SBOM are complete before production catalog inclusion.                                                                                                               |
...
| AC-MAN-014 | Freeze one shared candidate input, the Linux platform branch/result/index, and the later Windows platform branch/result/index; seal their aggregate evidence root; then build the production-purpose package from protected inputs, compare it with the qualification branches, and after separately authorized upload download/install the identical runtime digest from the final GitHub Release origin. | The aggregate root proves one unchanged candidate core and two complete, non-mutating platform branches with coherent predecessors; only declared version/trust/catalog/origin/release metadata differs for promotion; final GitHub runtime parity and public Hugging Face model transport pass; production collection rejects fixture/qualification trust; legal/signing/publication approvals are authentic external evidence; no upload, publication, or release occurs without separate authorization. |
...
**QUAL-004, OPS-003** Remaining work has three non-overlapping completion boundaries:

1. Linux qualification is complete only after the production artifact pipeline and both transfer profiles are implemented; qualification-purpose trust and the loopback HTTPS runtime origin are isolated; one explicit shared candidate input and one exact Linux platform input/profile graph are frozen in the Section 9.6 order; deterministic Linux CPU/CUDA runtime packs and all six exact upstream models are admitted; the complete Linux CPU and available NVIDIA CUDA profile, real public Hugging Face transport, lifecycle/privacy/offline/cancellation/crash/resource suites, and exact predecessor AppImage downgrade pass; and the Linux platform result/evidence index is frozen against that Linux graph. Exact Windows package/runtime/profile inputs, production private keys, final GitHub runtime upload, legal publication approval, and representative Windows execution are not Linux-qualification completion criteria.
2. Windows qualification is complete only after it consumes the unchanged shared candidate input and read-only Linux branch, freezes its distinct Windows application/runtime/direct-engine/toolchain/predecessor input and complete CPU/CUDA profile graph before measurement, runs the representative Windows package/installer/Job-Object/PDH/privacy/offline/lifecycle/downgrade suite, and freezes the Windows platform result/evidence index. Linux evidence cannot substitute; Windows qualification SHALL NOT mutate or regenerate the shared input or Linux branch and SHALL NOT define production trust.
3. Aggregate production readiness is complete only after Task 21 seals `aggregateEvidenceDigest` over the unchanged shared candidate input and both platform graph/result/index branches; validates acceptance ownership; proves qualification-to-production delta restrictions; validates production key/legal/provenance/SBOM/notice/redistribution evidence; packages the production-purpose catalog; and—after separately authorized upload—verifies the identical runtime digest through the final GitHub Release origin plus the pinned public Hugging Face model origin. The aggregate gate reports every remaining release blocker; it does not fabricate credentials, legal approval, uploads, or platform passes and does not rerun or rewrite unchanged expensive qualification branches.

Known remaining technical inputs and actions SHALL therefore be explicit rather than summarized as one generic blocker:

- implement the strict streaming `restricted-tar-gzip-v1` reader and `pinned-raw-model-v1` materializer, compose download/resume/retry/cancel/update into production main/preload/renderer ownership, and preserve safe removal;
- implement strict next-schema `qualification`/`production` catalogs, disjoint keyrings/purposes, redirect policies, qualification packaging rejection in release collection, and unchanged Task 17 fixture bytes;
- add deterministic runtime pack/catalog/provenance/SBOM/notice producers and qualification-to-production manifest comparison;
- freeze an explicit candidate SemVer, UTC freeze timestamp, clean shared source identity, exact shared model/corpus/schema/policy inputs, and `candidateInputDigest`; the worktree package version is not release authority;
- for each platform, freeze exact application/runtime/direct-engine/toolchain/predecessor/catalog/origin inputs, then CPU/CUDA profiles and `platformGraphDigest` before measurements; bind all series/results/indexes forward-only to that graph;
- execute complete Linux qualification and its exact predecessor test, then unchanged-candidate Windows qualification;
- obtain real protected production signing/keyring inputs, runtime component redistribution approval, public-model/corpus legal/provenance approval, final GitHub runtime asset upload authorization, and publication/release authority only in the aggregate production-readiness phase.

Release labels remain independently gated:

- Linux NVIDIA and Linux CPU Production require their complete all-six-model Linux profiles;
- Windows NVIDIA and Windows CPU Production require separate complete all-six-model Windows profiles;
- AMD remains **Preview · Untested** and cannot be promoted without future representative physical AMD work;
- macOS remains **Planned · Unavailable** regardless of shared skeleton completion;
- missing production trust/legal/publication inputs block a release, not already valid platform qualification evidence.

No private signing material, upload, publication, tag, support-tier promotion, or release may be inferred from specification or task approval. Before implementation resumes, the approved plan and Tasks 19–21 SHALL be revised to consume the corrected acyclic graph, assign its schema/validator/producer/consumer updates, and remove every requirement for Task 19 to pre-freeze Windows-specific identities.
