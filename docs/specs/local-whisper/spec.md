...
Full and reviewed quantized `ggml` files are distinct variants.
...
Redistribution review is mandatory for every included CUDA/cuBLAS/cuDNN, ROCm/HIP, `whisper.cpp`, and model or quantized-model component. MIT licensing of upstream projects does not remove component notice, provenance, or vendor redistribution obligations.
...
Every Production matrix cell SHALL have a checked-in, versioned release-qualification profile approved before the label ships. The profile SHALL identify exact OS build/family, architecture, device/reference hardware, driver/runtime/ISA, engine/backend, runtime/model revisions and variants, fixture hashes/licenses, repetitions, and pass limits.
...
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| AC-MAN-001 | Available Linux NVIDIA laptop: install the pinned `whisperCpp` CUDA pack and run every Linux NVIDIA configuration claimed by its qualification profile, including hybrid-device selection.                                                           | Integrity, correct device/backend, load/warm-up, accuracy, performance, memory, repeated inference, unload, crash recovery, and cleanup all meet the profile. Evidence applies only to Linux NVIDIA.                           |
| AC-MAN-002 | Windows/Linux x64 CPU with GPU target explicitly not selected, using each OS's declared `whisperCpp` Production reference hardware/profile.                                                                                                          | No GPU initializes and every correctness/performance/resource/lifecycle limit passes before that OS CPU cell becomes Production.                                                                                               |
| AC-MAN-003 | Separate representative Windows x64 NVIDIA hardware, the `whisperCpp` CUDA pack, and the complete Windows qualification profile.                                                                                                                     | All limits pass before the Windows NVIDIA cell is labeled Production. Linux evidence cannot substitute.                                                                                                                        |
...
- Windows/Linux CPU Production: blocked per OS until each `whisperCpp` CPU profile passes.
