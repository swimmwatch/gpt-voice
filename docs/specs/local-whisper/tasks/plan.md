...
Specification baseline: approved `spec.md` revision 10. This revision preserves
the fixed `whisperCpp` implementation, completed Tasks 01–18, and the
Linux/Windows/aggregate Tasks 19–21 split. It replaces the circular revision-9
candidate/profile freeze with the approved shared-candidate/platform-branch/
aggregate DAG and removes every requirement for Task 19 to pre-freeze exact
Windows identities. No completed packet or support claim is reopened.
...
| [19 Linux Production Pipeline And Qualification](19_linux_qualification.md)                                                     | Repair the unfrozen qualification-v2 schemas, finish the production artifact/trust path, freeze the shared candidate input and Linux input/profile graph in forward order, and qualify all six models on Linux CPU and available NVIDIA CUDA without Windows-specific inputs. | 01–18                                      | `DL-004`, `ARCH-010`, `COMP-012`, `DIST-001`–`DIST-002`, `MODEL-011`, `PKG-011`, `SEC-014`, `REL-001`, `QUAL-001`–`QUAL-004`, `PRIV-005`, `OPS-002`–`OPS-003`; Linux evidence for applicable prior requirements; primary `AC-AUTO-064`–`AC-AUTO-070`, `AC-AUTO-072`; Linux `AC-MAN-001`, `AC-MAN-002`, `AC-MAN-004`–`AC-MAN-008`, `AC-MAN-013`, and technical inputs for `AC-MAN-014`                                                                                                                                                                               |
| [20 Windows Qualification](20_windows_qualification.md)                                                                         | Consume the unchanged shared candidate input and read-only Linux branch, freeze a distinct Windows input/profile graph on representative Windows x64, then execute every Windows CPU/CUDA, native, installer, lifecycle, privacy, resource, transport, and downgrade gate.    | 19                                         | Windows slices of `QUAL-001`–`QUAL-004`, `PRIV-005`, `MODEL-011`, `DIST-002`, and applicable prior requirements; supporting evidence for `AC-AUTO-064`–`AC-AUTO-070`, `AC-AUTO-072`; Windows `AC-MAN-002`–`AC-MAN-008`, `AC-MAN-013`, and technical inputs for `AC-MAN-014`                                                                                                                                                                                                                                                                                         |
| [21 Aggregate Production Readiness And Release Blockers](21_aggregate_and_release_blockers.md)                                  | Validate the unchanged shared core and both immutable platform branches, seal `aggregateEvidenceDigest`, prove qualification-to-production and protected external gates, and report every blocker without rerunning or rewriting unchanged platform profiles.                 | 19, 20                                     | Aggregate `REL-001`, `COMP-012`, `PKG-011`, `SEC-014`, `DIST-001`–`DIST-002`, `QUAL-001`–`QUAL-004`, `OPS-002`–`OPS-003`, and applicable prior requirements; primary `AC-AUTO-002`, `AC-AUTO-023`, `AC-AUTO-032`, `AC-AUTO-040`, `AC-AUTO-071`; reconciliation of all automated acceptance plus `AC-MAN-001`–`AC-MAN-014`                                                                                                                                                                                                                                           |
...
- Tasks 01–18 remain completed foundations. Revision 16 does not reopen or
  re-execute them.
...
- Task 19 preserves the already integrated production graph, repairs the
  unfrozen circular v2 qualification contracts in place, freezes one explicit
  SemVer/UTC `candidateInputDigest`, then freezes Linux `platformInputDigest`,
  profiles, `platformGraphDigest`, result, and evidence index in forward order.
  Qualification-purpose trust and loopback runtime origin are sufficient;
  exact Windows packages/runtimes/profiles, production private keys, final
  GitHub upload, and legal publication approval are not Task 19 inputs.
- Task 20 consumes the unchanged shared candidate input and read-only Linux
  graph/result/index. It owns exact Windows application/runtime/direct-engine/
  toolchain/predecessor inputs, profiles, platform graph, result, and evidence
  index. It cannot mutate the shared core, Linux branch, or production trust.
- Task 21 validates ownership and both immutable platform branches, seals the
...
- `TASK 19 TECHNICAL INPUT — approved transports`: runtime packs use
  ...
  Task 19 owns both strict materializers and the production command path.
- `TASK 19 QUALIFICATION TRUST`: freeze one explicit SemVer/UTC shared
  candidate input, then a Linux qualification-purpose catalog/keyring,
  single-use loopback HTTPS origin, platform input, profiles, and platform
  graph for the exact Linux runtime bytes. Fixture and production trust remain
  disjoint. Exact Windows branch inputs, final production keys, GitHub upload,
  and legal publication approval are not Task 19 prerequisites.
- `TASK 19 QUALIFICATION INPUTS`: deterministic CPU/CUDA runtime archives,
...
- `MANUAL GATE — prior binary`: determine the highest stable release before
  freeze. If no later stable exists, Task 19 uses
  `GPT-Voice-2.3.0.AppImage` SHA-256
  `80674b3a90222b51981fb43b5b757b7af9d3e38a5ff4ca41554ab965ae29f111`
  and Task 20 uses `GPT-Voice.Setup.2.3.0.exe` SHA-256
  `0e2aa1ea97ba357db6d35f53debd01ca1c6124ae10b9f537b2af4427a0328cd0`.
  Task 21 reconciles both; a current-code fixture is not evidence.
- `TASK 21 PRODUCTION GATES`: protected production keyring/catalog inputs,
  ...
  Task 21 may validate and report them but may not fabricate or perform upload,
...
Revision 16 was explicitly approved by durable decision
`approval.plan-revision-16` through the persistent `plan:local-whisper`
interview. Plan approval authorizes no packet execution,
commit, push, pull request, private signing, upload, publication, support
promotion, or release. Task 19 requires a fresh explicit
`incremental-implementation` authorization after approval; each invocation
executes exactly one packet, updates `todo.md` and `handoff.md`, and stops
