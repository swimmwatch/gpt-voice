...
- Exact production inputs, protected reviewers, signers, legal/provenance
  material, and non-clobbering recovery procedures are available through the
  existing protected environment and version-scoped Watch contract.
...
- Explicitly enable Task 32's preserved publication gate with
  `publish=true`, `release_tag=v2.4.0-alpha.1`, and the exact prior versioned
  candidate run ID only after all preceding protected gates pass. The
  publication job must download and reverify that run's complete candidate;
  every construction/signing/build/attestation job is skipped during
  promotion. No second build, partial application-only upload, or alternate
  publication path is allowed.
- Treat the private versioned candidate as non-installable recoverable staging.
  The publication job creates the absent tag/release, then streams every public
  final-origin asset and verifies exact byte, manifest inventory, origin, tag,
  platform, target, and applicability before the same workflow can become
  green.
- Seal `releaseStagingDigest`, publish the complete public prerelease from the
  unchanged candidate bytes, verify final public origin/state in the same
  protected workflow, and seal alpha `deploymentDigest`.
...
preparation; protected build/sign/freeze; preserving merge; promotion of the
prior candidate without rebuild; workflow-owned exact tag/publication; public
final-origin verification. One explicit six-hour
`local-whisper-alpha-release` invocation authorizes that closed sequence; the
agent does not ask again between normal phases. Later phases may consume only
immutable outputs of earlier phases and may never rebuild, re-sign, retimestamp,
repackage, regenerate catalogs, or mutate metadata.
...
- The preserved default-off publication job, explicitly enabled for alpha.1
  only after exact-candidate verification and bound to the prior candidate run.
- `.codex/process-watch/scenarios/local-whisper-alpha-release.watch.json` and
  its separate class-based Node.js orchestration bundle; neither changes
  application code nor adds dependencies.
...
- A defect before publication invalidates the attempt at its earliest changed
  digest and permits a forward alpha.1 repair within scope. A changed source
  requires a new exact release PR head and versioned candidate; the previous
...
- `MANUAL GATE`: explicitly invoke
  `$watch-process scenario=local-whisper-alpha-release timeout=6h`. This one
  version-scoped authorization covers atomic commits/normal pushes, both PRs,
  protected-environment approval, preserving merges, candidate/promotion
  dispatch, workflow-owned tag creation, and immutable prerelease publication.
- It does not authorize force-push, amend/rebase/squash, overwrite/delete,
  repository settings, deploy, platform smoke, Tasks 34/35, or any other
  release identity. A public tag/release failure stops and requires alpha.2
  planning.
...
exist and the protected publication workflow is green. Update `todo.md` and
`handoff.md`; stop before either platform smoke, artifact installation,
feedback selection, later-alpha planning, final source work, or support
promotion. Later explicit invocations are required for Tasks 34 and 35.
