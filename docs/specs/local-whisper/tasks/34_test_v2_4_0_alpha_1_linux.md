# 34 Test v2.4.0-alpha.1 on Linux

## Outcome

Run the bounded post-publication Linux alpha smoke against only
`v2.4.0-alpha.1` final-origin assets and seal one truthful immutable Linux
platform-smoke result/evidence index. A pass supplies Linux lineage evidence; a
valid test failure is preserved and blocks final. Perform no release mutation,
Windows test, feedback transition, or final work.
...
- The representative Linux x64 RTX 50 host is explicitly authorized and has a
  fresh task-owned application/runtime inventory plus private evidence root.
- The task receives no production signing/publication secret and no release
  write authority.

## Owned Requirements

- Linux slices of `CI-005`, `REL-001`, `REL-003`, `REL-004`,
  `QUAL-001`, `QUAL-004`, `QUAL-005`, `QUAL-007`, `COMP-004`,
  `COMP-013` and `OPS-004`.
- Linux slice of `AC-AUTO-082`.
- `AC-MAN-017`.

## In Scope

- Revalidate the alpha `deploymentDigest`, exact tag/prerelease state, every
  configured public same-tag Linux package (`AppImage`, `deb`, and `rpm`),
  Linux CPU pack, Linux `sm_120a-real` pack, required public trust and
  verification assets, and pinned public Hugging Face `base/full` model
  identity before selecting the package appropriate for the test host.
- Freeze one Linux `platformSmokeInputDigest` binding the public alpha
  deployment, exact assets, Linux x64 host class, CPU and representative RTX 50
  target, driver/runtime/device-proof class, deterministic non-private fixture,
  bounded algorithm/timeouts/cleanup, and privacy-safe tool identities.
- Start from a clean task-owned inventory and exercise, once per target:
  clean application install; exact runtime/model acquisition and validation;
  compatibility check; load/warm-up; one non-private deterministic
  transcription; unload; offline restart/reuse; and cleanup.
- Prove CPU initializes no GPU; RTX uses the selected physical device and no
  CPU/PTX/other-device fallback; both leave no worker/listener/orphan or
  retained task-owned allocation.
- Seal one canonical Linux result and privacy-safe evidence index whether the
  smoke passes or truthfully fails.
- Scrub only task-owned temporary state after evidence sealing.

## Out Of Scope

- Windows execution or any dependency on Task 35's state.
...
- Sealing `alphaAggregateDigest`, selecting feedback, planning alpha.2, or
  executing final.
- All-six-model FLEURS/direct-engine performance, resource, repetition, crash,
  or lifecycle diagnostics. Those remain optional and nonblocking.
- Private user recordings, personal application inventory, ambient models or
  runtimes, support promotion, RTX 30/40, AMD, or macOS work.
...
redirect rules, or a locally substituted pack fail before installation. The
pinned Hugging Face model object is the only external content origin.

The Linux workflow is isolated from pull requests and forks, uses a manually
...
identifiers, audio, transcripts, prompts, keys, private logs, or measurement
series.
...
- Task 34 binds only Linux. It neither reads nor writes the Windows
...
- Linux post-release smoke workflow and least-privilege policy tests.
- Shared alpha-smoke schema/validator plus Linux host, installer, origin,
  CPU/RTX 50 device-proof, offline-reuse, lifecycle, cleanup, and evidence
  adapters.
...
- The Linux slice of `AC-AUTO-082` accepts only the exact public alpha.1
  Linux installer/CPU/`sm_120a-real` set and rejects missing deployment,
...
- `AC-MAN-017` executes the full bounded Section 19.2 Linux CPU and RTX 50
  flow and seals a truthful result.
- A pass records bounded Linux CPU/RTX 50 functional evidence only; it makes no
  per-model performance/resource claim and cannot substitute for Windows.
- A valid failure preserves the public alpha, blocks final, and permits only a
  future next-alpha transition.

## Verification

Implement/register the target-aware Linux command where absent:

```bash
rtk npm run test:local-whisper:qualification
rtk npm run test:local-whisper:release-lifecycle
rtk npm run verify:local-whisper:qualification:alpha -- --platform=linux --target=v2.4.0-alpha.1
...
- `MANUAL GATE`: authorize the representative Linux x64 RTX 50 host, public
...
Mark Task 34 complete after one valid immutable Linux result is sealed, whether
Pass or Fail. Update `todo.md` and `handoff.md` with privacy-safe input,
result, and evidence digests plus status. Stop before commit, Windows work,
