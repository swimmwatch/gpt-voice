...
- English browser proof: Playwright now checks the 390px no-JavaScript player and mobile navigation fallbacks, the hydrated Sheet’s Escape/focus-return behavior, local-only enhanced-player assets, the fallback `details` navigation when hydration is unavailable, horizontal overflow from 320–1440 pixels, and reduced-motion visibility. Plyr starts only after hydration succeeds, so React cannot replace an already-enhanced native video. Its icon sprite is emitted locally rather than fetched from `cdn.plyr.io`.
...
- `npm run landing:test:e2e` (seven English browser scenarios pass, including blocked hydration/Plyr chunks plus overflow and reduced-motion coverage).
