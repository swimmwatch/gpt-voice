...
| `src/renderer/components/TranslateSection.tsx`                                        | Product bridge, translation, and steady Command Dock shots                     | Render the actual component with Russian selected, an inert language callback, and the deterministic `useI18n` alias. Preserve the existing flag asset and selector styling.                                                                                       |
...
| ---------------------- | --------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `bridgeReady`          | ChatGPT Web connected, idle, Russian target   | Empty structured prompt draft                                                                            |
...
| `translationSelection` | idle, Russian target                          | English prompt selected, then verified Russian result                                                    |
...
- Frame `2580`: reviewed Russian result appears below the English source with explicit language labels.
- The result must be human-reviewed before final render; the composition fixture stores the approved final string.
...
5. Translation shows F11, Russian target, processing, copied state, and a reviewed result without another tool.
...
8. Translation shows F11, the Russian target, processing, copied state, and a reviewed result while explaining that no separate translation tool is needed.
...
7. **Synthetic content:** Use the approved spoken/retry prompt, Russian target, and rough/Prettified security-review pair.
