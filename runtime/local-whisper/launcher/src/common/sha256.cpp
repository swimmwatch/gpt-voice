...
void Sha256::update(const unsigned char* bytes, std::size_t length) {
  if (finished_ || (bytes == nullptr && length != 0) ||
  ...
  while (length > 0) {
    const std::size_t copied = std::min(length, buffer_.size() - buffered_bytes_);
    std::copy_n(bytes, copied, buffer_.begin() + static_cast<std::ptrdiff_t>(buffered_bytes_));
