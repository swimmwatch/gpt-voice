...
Status: Active ledger. The isolated video project installs the exact Remotion packages listed below. No third-party
stock asset, font, music, sound effect, or audio file has been downloaded or imported.

## Remotion 4.0.483

The isolated video project pins the following packages to exact version `4.0.483`:

| Package                 | npm metadata license         | Intended use                    | Eligibility status                 |
| ----------------------- | ---------------------------- | ------------------------------- | ---------------------------------- |
| `remotion`              | `SEE LICENSE IN LICENSE.md`  | Core composition runtime        | Installed; owner approval recorded |
| `@remotion/cli`         | `SEE LICENSE IN LICENSE.md`  | Studio, stills, and renders     | Installed; owner approval recorded |
| `@remotion/media`       | Not returned by npm metadata | Local audio/video playback      | Installed; owner approval recorded |
| `@remotion/transitions` | `UNLICENSED`                 | Semantic scene transitions      | Installed; owner approval recorded |
| `@remotion/motion-blur` | `MIT`                        | Accent trails only              | Installed; owner approval recorded |
| `@remotion/effects`     | `UNLICENSED`                 | Background gradients/glow/shine | Installed; owner approval recorded |
| `@remotion/light-leaks` | `Remotion License`           | Three bounded transition leaks  | Installed; owner approval recorded |
| `@remotion/noise`       | `MIT`                        | Fixed-seed texture              | Installed; owner approval recorded |
| `@remotion/shapes`      | `MIT`                        | Flow nodes and simple geometry  | Installed; owner approval recorded |
| `@remotion/paths`       | `MIT`                        | Typed workflow connectors       | Installed; owner approval recorded |
| `@remotion/media-utils` | `MIT`                        | Live-sample waveform analysis   | Installed; owner approval recorded |
...
**Human decision:** On 2026-07-14, the project owner confirmed that Remotion may be used for GPT-Voice. The agent records this as the required eligibility confirmation and does not make an independent legal classification. The project installs only the exact version-pinned packages listed above.
